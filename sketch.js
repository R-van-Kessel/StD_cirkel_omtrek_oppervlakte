...
function generateQuestions() {
    ...
    let niceRadii = [2, 3, 4, 5, 6, 7, 8, 9, 10, 12];
    let shuffledRadii = shuffle(niceRadii.slice());
    ...
    for (let i = 0; i < 10; i++) {
        let useRadius    = random() < 0.5;
        let useArea      = random() < 0.5;
        let convertToDm  = random() < 0.5;

        let r = shuffledRadii[i % shuffledRadii.length];
        let d = r * 2;

        let answerRaw, answerVal, answerLabel, taskLabel, givenLabel;

        if (useArea) {
            answerRaw   = Math.PI * r * r;
            answerVal   = parseFloat(answerRaw.toFixed(1));
            answerLabel = toNL(answerRaw);         // geen eenheid: "78,5"
            taskLabel   = 'oppervlakte = ? cm²';
        } else {
            answerRaw   = 2 * Math.PI * r;
            answerVal   = parseFloat(answerRaw.toFixed(1));
            answerLabel = toNL(answerRaw);         // geen eenheid: "31,4"
            taskLabel   = 'omtrek = ? cm';
        }

        // Gegeven label: soms cm, soms dm
        if (useRadius) {
            givenLabel = convertToDm
                ? toNL(r / 10) + ' dm'
                : r + ' cm';
        } else {
            givenLabel = convertToDm
        ...
        answers.push({ value: answerVal, label: answerLabel });
    }
    ...
    for (let row = 1; row < 3; row++) {
        for (let col = 0; col < COLS; col++) {
            ...
            blocks.push({
                ...
                text:       q.givenLabel,
                givenLabel: q.givenLabel,
                taskLabel:  q.taskLabel,
                answer:     q.answer,
                radius:     q.radius,
                diameter:   q.diameter,
                isArea:     q.isArea,
                useRadius:  q.useRadius,
                isQuestion: true,
                isCorrect:  null,
...
function drawBlock(block) {
    ...
    // Hover transform voor vraagblokken
    ...
    if (block.isQuestion) {
        ...
        let vr = 40;                         // vaste visuele straal
        let cx = bx + CELL_SIZE / 2;
        let cy = by + 15 + vr;               // cirkel midden: 15px marge + straal

        // ── CIRKEL ──────────────────────────────────────────────
        fill(200, 225, 255, 100);
        stroke(0);
        strokeWeight(2);
        ellipse(cx, cy, vr * 2, vr * 2);
      // Middelpunt stip
        fill(0);
        noStroke();
        ellipse(cx, cy, 5, 5);
        ...
        // ── PIJL + LABEL ────────────────────────────────────────
        stroke(220,220,220); //kleur diameter lijn
        strokeWeight(2);
        fill(255);

        let aw = 5, ah = 3;   // pijlpunt afmetingen

        if (block.useRadius) {
            // STRAAL: horizontale lijn van middelpunt naar rechts DOOR de cirkel
            let x1 = cx;
            let x2 = cx + vr;
            let lineY = cy;
            ...
            // Pijlpunt rechts (aan de rand van de cirkel)
            triangle(x2, lineY,
                     x2 - aw, lineY - ah,
                     x2 - aw, lineY + ah);
            ...
            // Label ONDER de pijllijn, gecentreerd op de lijn
            fill(0);
            noStroke();
            textAlign(CENTER, TOP);
            textSize(11);
            textStyle(BOLD);
            text(block.givenLabel, cx + vr / 2, lineY + 5);

        } else {
            // DIAMETER: horizontale pijl ONDER de cirkel
            let x1 = cx - vr;
            let x2 = cx + vr;
            let lineY = cy + vr + 6;        // 10px onder de cirkel
            ...
            // Verticale streepjes aan uiteinden (maataanduiding)
            // Stippellijn aan
            drawingContext.setLineDash([5, 4]);   // [streepje, tussenruimte]
            line(x1, lineY, x2, lineY);  
            stroke(220,220,220);
            strokeWeight(1.5);
            line(x1, lineY - 45, x1, lineY );
            line(x2, lineY - 45, x2, lineY );
           // Stippellijn uit (altijd resetten!)
            drawingContext.setLineDash([]);
            // Label ONDER de diameterpijl
            fill(0);
            noStroke();
            textAlign(CENTER, TOP);
            textSize(11);
            textStyle(BOLD);
            text(block.givenLabel, cx, lineY -5);
        }
        ...
        textStyle(BOLD);
        textLeading(15);
        text(block.taskLabel, bx + CELL_SIZE / 2, by + CELL_SIZE - 20);
